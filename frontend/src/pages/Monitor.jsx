...
export default function Monitor() {
  ...
  const showAlert = (eventObj) => {
    const eventType = eventObj.eventType?.toUpperCase() || '';
    const severity = eventObj.severity?.toUpperCase() || '';

    // 只有 severity 为 CRITICAL/WARNING/NORMAL
    ...
    let toastClass = isCritical ? 'danger' : isWarning ? 'warning' : 'normal';
  ...
  useEffect(() => {
    const fetchEvents = () => {
      axios
        .get(API_BASE + '/events/list?size=1000')
        .then((res) => {
        ...
        <div className="events-container">
          {events.length === 0 ? (
            ...
            events.map((evt, index) => {
              ...
              const severity = evt.severity?.toUpperCase() || '';
              const isCritical = severity === 'CRITICAL';
              const isWarning = severity === 'WARNING';
              const cardClass = isCritical ? 'danger' : isWarning ? 'warning' : 'normal';

              return (
                <div key={index} className={`event-card ${cardClass}`}>
